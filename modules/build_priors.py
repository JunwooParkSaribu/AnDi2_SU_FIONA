...
def make_priors(datapath, savepath, imagepath, prefix, exp, make_image):
    reg_model_nums = [5, 8, 12, 16, 32, 64, 128]
    ext_width = 100
    win_widths = np.arange(20, 40, 2)
    shift_width = 5
    if not os.path.exists(f'{savepath}/priors/priors_{prefix}_{exp}.npz'):
        reg_model = RegModel(reg_model_nums)
        all_seg_lengths = []
        all_alphas = []
        all_ks = []
        dfs, _, _ = load_andi_files.load_datas(datapath, prefix, exp)
        for df in dfs:
        ...
        np.savez(f'{savepath}/priors/priors_{prefix}_{exp}.npz',
                 alphas=all_alphas, seg_lengths=all_seg_lengths, all_ks=all_ks)

        if make_image:
            #raw_distribution(f'{imagepath}/track{prefix}_exp{exp}_sample_distribs.png', all_alphas, all_ks, all_seg_lengths)
            print('Sample distribution generated...')


if __name__ == "__main__":
    make_priors(datapath=sys.argv[1], savepath=sys.argv[2], imagepath=sys.argv[3],
                prefix=int(sys.argv[4]), exp=int(sys.argv[5]), make_image=bool(sys.argv[6]))
